...
class NewsAdapterVertical(article: List<Article>, private val activity: AppCompatActivity) :
    RecyclerView.Adapter<NewsAdapterVertical.MyViewHolder>() {
    ...
    private var article: List<Article>
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val view =
            LayoutInflater.from(activity).inflate(R.layout.news_items_vertical, parent, false)
    ...
    override fun onBindViewHolder(holders: MyViewHolder, position: Int) {
        ...
        holders.favouriteBtn.setOnClickListener {
            if (getUserLogin.status == "login") {
                newsFavouriteByUser.addFavouriteByUser(
                    getUserLogin.userID, model.url, model.title,
                    path, model.publishedAt
                )
        ...
        holders.unFavouriteBtn.setOnClickListener {
            if (getUserLogin.status == "login") {
                newsFavouriteByUser.removeFavouriteByUser(getUserLogin.userID, "", model.title)
                holders.favouriteBtn.visibility = View.VISIBLE
                holders.unFavouriteBtn.visibility = View.GONE
            } else {
                Toast.makeText(
