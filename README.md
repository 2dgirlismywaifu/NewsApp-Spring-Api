...
<p1 align="center", style="font-size:20px">Powered by NewsAPI and Spring Services</p1><br />
...
News application or newspaper reading application is a popular application with the role of
providing quick news to users. In this project, the application uses: NewsAPI with 53 countries
supported, RSS2JSON converts RSS to JSON and Microsoft Azure services: SQL Server, App Services and
Blob Storages.
...
- At this time, the application is only developed on the Android platform. Once I have a Mac device,
  I will develop this project run on Apple mobile device
...
* Create App Services B1 Plan (Recomended:
...
1. Clone the repo
   ...
   ```
2. Follow instruction from my App
   Services: [NewsApp Android RESTServices](https://github.com/2dgirlismywaifu/NewsApp_Android_RESTServices)
...
5. Get a secret access token to Azure Blob Storage follow this
   link [Create SAS Token Azure Blob Containers](https://learn.microsoft.com/en-us/azure/cognitive-services/translator/document-translation/how-to-guides/create-sas-tokens?tabs=Containers)
6. Open this project in Android Studio
7. Now connect this project to Firebase Authentication\
   The easy way: In Menu at top your screen, choose Tools -> Firebase -> Authentication. Follow this
   video bellow to see details\
...
- Wait for this command run and you will get SHA-1 and SHA-256 code return like bellow
  ...
  ```
- Open your project firebase connect with this project, scroll down to your app part. In SHA
  certificate fingerprints, add SHA-1 and SHA-256 code project have taken above.
...
9. Now open Firebase Authentication in your Firebase Project
    ...
    - In Settings part, choose `User account linking` ,
      select `Create multiple accounts for each identity provider`. Then select `Save` to complete
...
1. RecycleView Search only work with RecycleView Horizontal (expect: NewsSourceList, Favourite
   News).
2. Set favourite an un favourite in recycleview adapter not update view. Need Swipe To Refresh
   action from user to update view.
3. ~~Duplicate result login~~
4. Tell me 💕\
   See the [open issues](https://github.com/2dgirlismywaifu/NewsAPP_RSS_NewsAPI_Azure/issues) for a
...
Project
Link: [https://github.com/2dgirlismywaifu/NewsAPP_RSS_NewsAPI_Azure](https://github.com/2dgirlismywaifu/NewsAPP_RSS_NewsAPI_Azure)
\
...
<h3 align="center", style="font-size:25px">NewsApp Reader (Curently working update API URI)</h3>
